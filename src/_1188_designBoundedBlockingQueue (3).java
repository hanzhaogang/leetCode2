...
public class _1188_designBoundedBlockingQueue {
	...
	public _1188_designBoundedBlockingQueue(int capacity) {
    ...
    public void enqueue(int element) throws InterruptedException {
    	lock.lock();
    	try{
    		while(q.size()==capacity) {//await may be break by other signals, so use while loop
    			full.await();//this will release the lock& wait.
    		}
    		q.offer(element);
    		empty.signal();
    	}finally {
    		lock.unlock();//unlock always be called in the finally block
    	}
    }
