 ...
 * The program source is an array 
 * where source[i] is the i-th line of the source code. 
...
it and rest of the characters to the right of it in the same line 
should be ignored.
...
should be ignored. 
(Here, occurrences happen in reading order: 
...
you must not output that line: 
each string in the answer list will be non-empty.

There will be no control characters, single quote, 
or double quote characters. 
For example, source = "
string s = "/* Not a comment. * /";            " 
will not be a test case. 
(Also, nothing else such as defines or macros 
 ...
 */
 Carefully parse each line according to the following rules: 
	 ...
	 * If we end a block comment and we are in a block, 
	 ...
	 change our state to be *not* in a block. 
	 ...
	 * If we aren't in a block comment 
	 (and it wasn't the start of a comment), 
			 ...
			 * At the end of each line, 
			 if we aren't in a block, we will record the line.
