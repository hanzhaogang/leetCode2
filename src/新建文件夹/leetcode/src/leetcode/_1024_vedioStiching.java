...
Constraints:
    ...
    0 <= T <= 100
 */
public class _1024_vedioStiching {
	
	public static void main(String[] args) {
		int[][] clips=new int[][] {{0,2},{1,3},{1,4},{1,8}};
		int T=5;
		_1024_vedioStiching s=new _1024_vedioStiching();
    ...
    public int videoStitching(int[][] clips, int T) {
    	...
    	int count=0;
    	int nextHi=0;
    	for(int i=0;i<clips.length;) {//[0,2][4,8] 10//[0,4][2,8] 5//[0,1][1,2] 5
    		if(hi<clips[i][0]) {
    			return -1;
    		}
    		while(i<clips.length&&clips[i][0]<=hi) {//note index out of range
    			if(T<=clips[i][1]) {
    				return count+1;
    			}
    			if(nextHi<clips[i][1]) {
