...
The value of each color in image[i][j] and newColor will be an integer in [0, 65535].
	...
	private void dfs(int[][] image,int x,int y,int oriColor,int newColor) {
		if(x<0||image.length<=x||y<0||image[0].length<=y) {
			return;
		}
		if(image[x][y]!=oriColor||visited[x][y]) {
			return;
		}
		...
		image[x][y]=newColor;
		for(int[] dir:dirs) {
			dfs(image,x+dir[0],y+dir[1],oriColor,newColor);
		}
	}
}
