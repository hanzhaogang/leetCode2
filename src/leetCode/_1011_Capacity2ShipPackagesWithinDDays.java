 ...
 * that must be shipped from one port to another within D days.
...
Constraints:
    ...
    1 <= weights[i] <= 500
 */
public class _1011_Capacity2ShipPackagesWithinDDays {
    public int shipWithinDays(int[] weights, int D) {
    	...
    	return bs(weights,D,minCap,maxCap);
    }
    
    private int bs(int[] weights,int D,int lo,int hi) {
    	if(hi<lo)
    		return -1;
    	int mid=lo+(hi-lo)/2;
    	if(timing(weights,mid)<=D&&(mid==1 || D<timing(weights,mid-1))) {
    		return mid;
    	}else if(timing(weights,mid)<=D&&timing(weights,mid-1)<=D) {
    		return bs(weights,D,lo,mid-1);
    	}else {
    		return bs(weights,D,mid+1,hi);
    	}
    }
    
    private int timing(int[] weights,int cap) {
    	int days=1;
    	int leftCap=cap;
    	for(int weight:weights) {
    		if(leftCap<weight) {
